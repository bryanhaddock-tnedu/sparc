...
All authorized Product viewers may also receive the Task Level Forecast Breakdown: monthly non-Epic Jira tickets with actual work, their key and summary, work type, story points, ticket estimate, actual cost, and variance. Receipts never expose contributor names, role-level worklog breakdowns, or bill rates. Ticket estimates use an Admin-maintained Jira Team Estimation Profile selected by the ticket's Jira Team value. Admins can create, edit, or remove Jira Team Estimation Profiles from Estimation Settings. These profiles are independent from SPARC Team rosters, use Jira-Team-specific velocity/capacity assumptions, and calculate planning rates from active Team Members with positive approved rates by role; they do not change individual-contributor Forecast entry. The Product Maintenance Jira Team profile is seeded with its approved starting assumptions so UAT and future deploys can calculate estimates immediately unless an Admin-maintained profile already exists. Epics are treated as containers and excluded from the normal task list once Jira issue type is known on synced actual worklogs.
